    ...
    if ( ! class_exists( 'ReduxFramework_extension_customizer' ) ) {
        ...
        class ReduxFramework_extension_customizer {
            ...
            public function field_render( $option ) {
                echo esc_html__('1','cityestate');
                preg_match_all( "/\[([^\]]*)\]/", $option->id, $matches );
                $id = $matches[1][0];
                echo ecs_html($option->link());

                $this->parent->_field_input( $this->controls[ $id ] );
                echo esc_html__('2','cityestate');
            }
            ...
            public function _enqueue_previewer() {
                ...
                $localize = array(
                    'save_pending'   => __( 'You have changes that are not saved. Would you like to save them now?', 'cityestate' ),
                    'reset_confirm'  => __( 'Are you sure? Resetting will lose all custom values.', 'cityestate' ),
                    'preset_confirm' => __( 'Your current options will be replaced with the values of this preset. Would you like to proceed?', 'cityestate' ),
