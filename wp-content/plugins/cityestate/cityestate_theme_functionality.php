<?php
/*
Plugin Name: City Estate Theme - Functionality
Plugin URI:  https://themeforest.net/user/fortunecreations
Description: Adds functionality to Fortune Creations Themes
Version:     1.0
Author:      Fortune Creations
Author URI:  https://themeforest.net/user/fortunecreations
License:     GPL2
*/

class CityEstate_Theme_Functionality {

	// Cityestate class constructor
	public function __construct(){     	
     	// Cityestate call constants function
        $this->cityestate_constants();

        // Cityestate call include files function
    	$this->cityestate_include_files();

        // User login with social media
        //require_once( CITYESTATE_PATH . 'social-login/social_login.php' );

        // Call cityestate plugin activation and deactivate hooks
        register_activation_hook( __FILE__, array( &$this, 'cityestate_plugin_activation' ) );
        register_deactivation_hook( __FILE__, array( &$this, 'cityestate_plugin_deactivate' ) );
    }

    // Cityestate define constants     
    protected function cityestate_constants(){
        // Cityestate set plugin path
        define( 'CITYESTATE_PATH', plugin_dir_path( __FILE__ ) );
    }
    
    // Cityestate call function after plugin is activate
    public function cityestate_plugin_activation(){
        // Cityestate add extra table for advance search property
        global $wpdb;
        $cityestate_table_name          = $wpdb->prefix . 'cityestate_search';
        $cityestate_charset_collate     = $wpdb->get_charset_collate();
        $cityestate_add_tbl_query       = "CREATE TABLE $cityestate_table_name ( id mediumint(11) NOT NULL AUTO_INCREMENT, auther_id mediumint(11) NOT NULL, query longtext NOT NULL, email longtext DEFAULT '' NOT NULL, url longtext DEFAULT '' NOT NULL, time datetime DEFAULT '0000-00-00 00:00:00' NOT NULL, UNIQUE KEY id (id) ) $cityestate_charset_collate;";
        // Cityestate call upgrade hook
        require_once( ABSPATH . 'wp-admin/includes/upgrade.php' );
        dbDelta( $cityestate_add_tbl_query );
    }

    // Cityestate call when plugin is deactive
    public function cityestate_plugin_deactivate(){
        // Cityestate remove extra table
        global $wpdb;
        $cityestate_table_name      = $wpdb->prefix . 'cityestate_search';
        $cityestate_remove_tbl_qur  = "DROP TABLE ". $cityestate_table_name;
        
        // Cityestate call query
        $wpdb->query( $cityestate_remove_tbl_qur );
    }

    // Cityestate include files like metabox, theme option etc.
    function cityestate_include_files(){
        // Cityestate get theme name
        $ce_theme_name = wp_get_theme()->Name;

        // Cityestate check theme is Cityestate or Cityestate Child
        if( $ce_theme_name == 'Cityestate' || $ce_theme_name == 'Cityestate Child' ){
            
            // Cityestate property type custom post
            require_once( CITYESTATE_PATH . 'custom-post/cityestate_property.php' );
            // Cityestate agent type custom post
            require_once( CITYESTATE_PATH . 'custom-post/cityestate_agent.php' );
            // Cityestate membership type custom post
            require_once( CITYESTATE_PATH . 'custom-post/cityestate_package.php' );
            // Cityestate invoice type custom post
            require_once( CITYESTATE_PATH . 'custom-post/cityestate_invoice.php' );
            // Cityestate testimonial type custom post
            require_once( CITYESTATE_PATH . 'custom-post/cityestate_testimonial.php' );
            
            // Cityestate call visual composer custom element
            ...
            // Cityestate call metabox for agent, membership, page, partner, property, testimonial etc.
            if( !class_exists('RW_Meta_Box') ){
                require_once( CITYESTATE_PATH . 'framework/meta-box/meta-box.php' );
                
                // Cityestate add metabox column extension
                if( !class_exists('RWMB_Columns') ){
                    require_once( CITYESTATE_PATH . 'framework/meta-box/addons/meta-box-columns/meta-box-columns.php' );
                }

                // Cityestate add metabox group extension
                if ( !class_exists('RWMB_Group') ){
                    require_once( CITYESTATE_PATH . 'framework/meta-box/addons/meta-box-group/meta-box-group.php' );
                }

                // Cityestate add metabox tabs extension
                if ( !class_exists('RWMB_Tabs') ){
                    require_once( CITYESTATE_PATH . 'framework/meta-box/addons/meta-box-tabs/meta-box-tabs.php' );
                }                
            }

            // Cityestate call metaboxes
            if( class_exists('RW_Meta_Box') ){
                require_once( CITYESTATE_PATH . 'framework/meta-box/cityestate_meta_boxes.php' );
            }

            // Cityestate call taxonomy metabox
            if( ! class_exists( 'Tax_Meta_Class') ){
                require_once( CITYESTATE_PATH . 'framework/Tax-meta-class/Tax-meta-class.php' );
                require_once( CITYESTATE_PATH . 'framework/Tax-meta-class/cityestate_tax_meta_config.php' );
            }

            // Cityestate call redux theme option
            if( !class_exists('ReduxFramework') ){
                require_once( CITYESTATE_PATH . 'framework/core/ReduxCore/framework.php' );
            }
        }
    }
}

// Cityestate instantiate the theme class
$CityEstate_Theme_Functionality = new CityEstate_Theme_Functionality();
...
// Print property
if( !function_exists('cityestate_print_property_detail') ){

    function cityestate_print_property_detail(){
        ...
        // Get property id and property object
        $property_id    = intval($_POST['property_id']);
        $property_post  = get_post( $property_id );

        // Check security
        if( $property_post->post_type != 'property' || $property_post->post_status != 'publish' ){
            exit( 'are you kidding?' );
        }

        // Project logo
        $site_logo = cityestate_option( 'property_site_logo', 'url' );

        // Get property media and content
        $property_title     = get_the_title( $property_id );
        $property_detail    = get_page( $property_id );
        $property_content   = $property_detail->post_content;
        $image_id           = get_post_thumbnail_id( $property_id );
        $property_image     = wp_get_attachment_image_src( $image_id, 'full' );
        $property_image     = $property_image[0];
        
        // Get property detail
        $property_images        = get_post_meta( $property_id, 'property_images', false );
        $property_address       = get_post_meta( $property_id, 'property_map_address', true );
        $property_size          = get_post_meta( $property_id, 'property_size', true );
        $property_size_prefix   = get_post_meta( $property_id, 'property_size_prefix', true );
        ...
        $property_year          = get_post_meta( $property_id, 'property_year', true );
        $property_garages       = get_post_meta( $property_id, 'property_garages', true );                
        $custom_id              = get_post_meta( $property_id, 'property_id', true );
        $property_price         = get_post_meta( $property_id, 'property_price', true );
        $property_floor         = get_post_meta( $property_id, 'floor_plans', true );
        $property_agent         = get_post_meta( $property_id, 'property_agent_display', true );
        $property_agent_id      = get_post_meta( $property_id, 'agents', true );

        if( $property_agent_id != '-1' && $property_agent == 'agent_info' ){
            
            $agent_website      = get_post_meta( $property_agent_id, 'agent_website', true );
            $agent_email        = get_post_meta( $property_agent_id, 'agent_email', true );
            $agent_mobile       = get_post_meta( $property_agent_id, 'agent_mobile', true );
            $agent_phone        = get_post_meta( $property_agent_id, 'agent_office', true );
            $agent_fax          = get_post_meta( $property_agent_id, 'agent_fax', true );
            $agent_skype        = get_post_meta( $property_agent_id, 'agent_skype', true );
            $agent_name         = get_the_title( $property_agent_id );
            $thumb_id           = get_post_thumbnail_id( $property_agent_id );
            $thumb_url_array    = wp_get_attachment_image_src( $thumb_id, 'thumbnail', true );
            $agent_photo_url    = $thumb_url_array[0];

        } else if( $property_agent == 'author_info' ){

            $agent_fax          = get_post_meta( $property_agent_id, 'author_fax', true );
            $agent_phone        = get_the_author_meta( 'author_phone', $author_id );
            $author_id          = get_post_field( 'post_author', $property_id );
            $agent_name         = get_the_author_meta( 'display_name', $author_id );
            $agent_mobile       = get_the_author_meta( 'user_mobile', $author_id );
            $agent_skype        = get_the_author_meta( 'user_skype_id', $author_id );
            $agent_website      = get_the_author_meta( 'user_website_url', $author_id );
            $agent_photo_url    = get_the_author_meta( 'user_custom_image', $author_id );
            $agent_email        = get_the_author_meta( 'email', $author_id );
        }
        ...
        $show_print_property_desctiption  = cityestate_option( 'show_print_property_desctiption' );
        $show_print_property_basic_detail = cityestate_option( 'show_print_property_basic_detail' );
        $show_print_property_gallery      = cityestate_option( 'show_print_property_gallery' );
        $show_print_property_agent        = cityestate_option( 'show_print_property_agent' );
        $print_floor_plans                = cityestate_option( 'show_print_property_floor_plan' );
        ...
        print '<html><head><link href="'.get_template_directory_uri().'/css/print-property.css" rel="stylesheet" type="text/css" />';
                    ...
                    <div class="detail-block">
                        ...
                        <div class="print-header-detail">
                            <div class="print-header-detail-left">
                                <h1><?php echo esc_attr( $property_title ); ?></h1>
                                <p><?php echo esc_attr( $property_address ); ?></p>
                            </div>
                            <div class="print-header-detail-right"><?php
                                $sale_price     = get_post_meta( $property_id, 'property_price', true );
                                $second_price   = get_post_meta( $property_id, 'property_second_price', true );
                                $price_postfix  = get_post_meta( $property_id, 'property_price_postfix', true );
                                ...
                                if( !empty($sale_price) ){
                                    ...
                                    if( !empty($sale_price) && !empty($second_price) ){
                                        ...
                                        $output .= '<h1>' . cityestate_filter_property_price($sale_price) . '</h1>';
                                        if( !empty($second_price) ){
                                            $output .= '<h4>';
                                            $output .= cityestate_filter_property_price($second_price) . $price_postfix;
                                            $output .= '</h4>';
                                        }
                                    } else {
                                        if( !empty( $sale_price ) ){
                                            $output .= '<h1>';
                                            $output .= cityestate_filter_property_price($sale_price) . $price_postfix;
                                            $output .= '</h1>';
                                        }
                                    }
                        ...
                        <div class="print-banner">
                            <div class="print-main-image">
                                <?php if( !empty($property_image) ){ ?>
                                    ...
                                    <img class="qr-image" src="https://chart.googleapis.com/chart?chs=105x104&cht=qr&chl=<?php echo esc_url( get_permalink($property_id) ); ?>&choe=UTF-8" title="<?php echo esc_attr($property_title); ?>" />
                                <?php } ?>
                            </div>
                        </div>

                        <?php if( $show_print_property_desctiption != 0 ){ ?>
                        <div class="print-block">
                            <div class="detail-title-inner">
                                <h4 class="title-inner"><?php esc_html_e( 'Property Description', 'cityestate' ); ?></h4>
                            </div>
                            <p><?php echo esc_html($property_content); ?></p>
                        </div>
                        <?php } ?>

                        <?php if( $show_print_property_basic_detail != 0 ){ ?>
                        <div class="print-block">
                            <div class="detail-title-inner">
                                <h4 class="title-inner"><?php esc_html_e( 'Property Details', 'cityestate' ); ?></h4>
                            </div>
                            <div class="alert">
                                <ul class="print-list-three-col"><?php
                                    if( !empty($custom_id) ){
                                        echo '<li><strong>'.esc_html__( 'Property ID:', 'cityestate' ).'</strong> '.esc_attr( $custom_id ).'</li>';
                                    }
                                    if( !empty($property_price) ){
                                        ...
                                        if( !empty($sale_price) ){
                                            ...
                                            echo '<li><strong>'.esc_html__( 'Price:', 'cityestate' ). '</strong> '.cityestate_filter_property_price($sale_price) . $price_postfix.'</li>';
                                        }
                                    }

                                    if( !empty($property_size) ){
                                        echo '<li><strong>'.esc_html__( 'Property Size:', 'cityestate' ). '</strong> '.$property_size . ' '. $property_size_prefix.'</li>';
                                    }

                                    if( !empty($property_bedrooms) ){
                                        echo '<li><strong>'.esc_html__( 'Bedrooms:', 'cityestate' ).'</strong> '.esc_attr( $property_bedrooms ).'</li>';
                                    }

                                    if( !empty($property_bathrooms) ){
                                        echo '<li><strong>'.esc_html__( 'Bathrooms:', 'cityestate' ).'</strong> '.esc_attr( $property_bathrooms ).'</li>';
                                    }

                                    if( !empty($property_garages) ){
                                        echo '<li><strong>'.esc_html__( 'Garage:', 'cityestate' ).'</strong> '.esc_attr( $property_garages ).'</li>';
                                    }

                                    if( !empty($property_year) ){
                                        echo '<li><strong>'.esc_html__( 'Year Built:', 'cityestate' ).'</strong> '.esc_attr( $property_year ).'</li>';
                                    } ?>
                                </ul>
                            </div>
                            <?php } ?>

                            <?php if( $additional_features_enable != 'disable' && !empty( $additional_features_enable ) && $show_print_property_basic_detail_additional != 0 ) { ?>
                            <div class="detail-title-inner">
                                <h4 class="title-inner"><?php esc_html_e( 'Additional details', 'cityestate' ); ?></h4>
                            </div>
                            <ul class="print-list-three-col">
                                <?php
                                foreach( $additional_features as $ad_del ):
                                    echo '<li><strong>'.esc_attr( $ad_del['additional_feature_title'] ).':</strong> '.esc_attr( $ad_del['additional_feature_value'] ).'</li>';
                                endforeach;
                                ?>
                            </ul>
                            <?php } ?>
                        ...
                        <div class="print-block"><?php
                            $amenities = get_post_meta( $property_id, 'propertyamenities', true );
                            if( !empty($amenities) ){ ?>
                            ...
                            $property_info = get_post_meta( $property_id, 'property_info', true );
                            ...
                            $flooring_detail = get_post_meta( $property_id, 'flooring_detail', true );
                            $goods_detail = get_post_meta( $property_id, 'goods_detail', true );
                            ...
                            $interior_exterior = get_post_meta( $property_id, 'interior_exterior', true );
                            ...
                            $room_dimensions = get_post_meta( $property_id, 'room_dimensions', true ); 
                        ...
                        <?php if( !empty( $property_floor ) && $print_floor_plans != 0 ){ ?>
                        <div class="print-floor">
                            <div class="detail-title-inner">
                                <h4 class="title-inner"><?php esc_html_e( 'Floor Plans', 'cityestate' ); ?></h4>
                            </div>
                            <div class="accord-block">

                                <?php foreach( $property_floor as $plan ): ?>
                                    <div class="accord-outer">
                                        <div class="accord-tab">
                                            <h3><?php echo esc_attr( $plan['floor_plan_title'] ); ?></h3>
                                            <ul>
                                                <?php if( !empty( $plan['floor_plan_size'] ) ) { ?>
                                                    <li><?php esc_html_e( 'Size:', 'cityestate' ); ?> <strong><?php echo esc_attr( $plan['floor_plan_size'] ); ?></strong></li>
                                                <?php } ?>

                                                <?php if( !empty( $plan['floor_plan_room'] ) ) { ?>
                                                    <li><?php esc_html_e( 'Rooms:', 'cityestate' ); ?> <strong><?php echo esc_attr( $plan['floor_plan_room'] ); ?></strong></li>
                                                <?php } ?>

                                                <?php if( !empty( $plan['floor_plan_bathroom'] ) ) { ?>
                                                    <li><?php esc_html_e( 'Baths:', 'cityestate' ); ?> <strong><?php echo esc_attr( $plan['floor_plan_bathroom'] ); ?></strong></li>
                                                <?php } ?>

                                                <?php if( !empty( $plan['floor_plan_price'] ) ) { ?>
                                                    <li><?php esc_html_e( 'Price:', 'cityestate' ); ?> <strong><?php echo esc_attr( $plan['floor_plan_price'] ); ?></strong></li>
                                                <?php } ?>
                                            </ul>
                                        </div>
                                        <div class="accord-content">
                                            <?php if( !empty( $plan['floor_plan_image'] ) ) { ?>
                                                <img src="<?php echo esc_url( $plan['floor_plan_image'] ); ?>" alt="<?php echo esc_attr($plan['floor_plan_title']); ?>">
                                            <?php }
                                            if( !empty( $plan['floor_plan_info'] ) ) { ?>
                                                <p><?php echo esc_attr( $plan['floor_plan_info'] ); ?></p>
                                            <?php } ?>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                        ...
                        <?php if( $show_print_property_agent != 0 ){ ?>
                        <div class="print-block">
                            <div class="media agent-media">
                                <div class="media-left">
                                    <a href="#">
                                        <img src="<?php echo esc_url( $agent_photo_url ); ?>" class="media-object" alt="image" height="74" width="74">
                                    </a>
                                </div>
                                <div class="media-body">
                                    <h4 class="media-heading"><?php esc_html_e( 'Contact Agent', 'cityestate' ); ?></h4>
                                    <ul>
                                       <!--  <li><strong><?php echo esc_attr($prop_agent); ?></strong></li> -->
                                        <li><strong><?php esc_html_e( 'Mobile:', 'cityestate' ); ?></strong> <?php esc_attr($agent_mobile,'cityestate'); ?></li>
                                        <li><strong><?php esc_html_e( 'Email:', 'cityestate' ); ?></strong> <?php esc_attr($agent_email,'cityestate'); ?></li>
                                        <li><strong><?php esc_html_e( 'Fax:', 'cityestate' ); ?></strong> <?php esc_attr($agent_fax,'cityestate'); ?></li>
                                        <li><strong><?php esc_html_e( 'Phone:', 'cityestate' ); ?></strong> <?php esc_attr($agent_phone,'cityestate'); ?></li>
                                        <li><strong><?php esc_html_e( 'Skype:', 'cityestate' ); ?></strong> <?php esc_attr($agent_skype,'cityestate'); ?></li>
                                        <li><strong><?php esc_html_e( 'Website:', 'cityestate' ); ?></strong> <?php echo esc_url($agent_website,'cityestate'); ?></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        ...
                        <?php if( !empty( $property_images ) && $show_print_property_gallery != 0 ) { ?>
                        <div class="print-gallery">
                            <div class="detail-title-inner">
                                <h4 class="title-inner"><?php esc_html_e( 'Property images', 'cityestate' ); ?></h4>
                            </div>
                            <?php foreach( $property_images as $image_id ): ?>
                                <div class="print-gallery-image"> <?php echo wp_get_attachment_image( $image_id, 'cityestate_property_slider_image' ); ?> </div>
                            <?php endforeach; ?>
                        </div>
                        <?php } ?>                        
                    </div>
...
add_action( 'wp_ajax_nopriv_cityestate_print_property_detail', 'cityestate_print_property_detail' );
add_action( 'wp_ajax_cityestate_print_property_detail', 'cityestate_print_property_detail' );


// encode string with base64_encode
if( !function_exists('cityestate_encode') ){

    function cityestate_encode( $data ){
...
if( !function_exists('cityestate_decode') ){
    ...
    function cityestate_decode( $data ){
